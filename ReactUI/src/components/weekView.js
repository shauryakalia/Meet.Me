...
export default function WeekView(props) {
    ...
    const addSlot = async (index, time) => {
        try {
            ...
            let fromTime = `${selectedDate.getFullYear()}-${monthCheck < 10 ? '0'+ monthCheck : monthCheck }-${selectedDate.getDate() < 10 ? '0' + selectedDate.getDate() : selectedDate.getDate()}T${hour}:${minute}:01.000Z`
            const practiceId = localStorage.getItem('userId');
